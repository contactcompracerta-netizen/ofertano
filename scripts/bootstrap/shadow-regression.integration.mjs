...
try {
  ...
  await scaffoldLocalSupabase(client,url(forward));
  ...
  assert.equal(ledger.length,Object.keys(manifest.baselineMigrations).length+Object.keys(manifest.forwardMigrations).length);for(const row of ledger){assert.ok(row.finished_at);assert.equal(row.applied_steps_count,manifest.baselineMigrations[row.migration_name]?0:1);}
  ...
  for(const name of [fresh,forward]){const schema=localEquivalenceSchema('prisma/schema.prisma');try{run(['node_modules/prisma/build/index.js','migrate','diff','--from-config-datasource','--to-schema',schema.file,'--exit-code'],name);}finally{schema.cleanup();}assert.match(run(['scripts/bootstrap/fresh-bootstrap.mjs','--check'],name),/"category":"B"/);}
